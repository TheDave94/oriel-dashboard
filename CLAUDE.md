...
5. **`verify-install`** (second job in the same workflow) replays a HACS plugin install against the published release with `tools/hacs-install-sim.py` and fails the run if any runtime chunk the entry bundle requests is missing. Branches on the script's exit code: 1 (missing chunk) fails fast, 3 (transport) retries, 2 (structural) retries only while the release asset list still lags `dist/`. See [docs/RELEASE-INFRASTRUCTURE.md](docs/RELEASE-INFRASTRUCTURE.md#post-release-install-verification-gate).
6. **HACS users fetch the release assets**, not the source tree. The source-tree `dist/` is not load-bearing for users — release-build.yml builds it fresh on every release.
...
| File | When to load |
|------|--------------|
| `CONVENTIONS.md` | Before writing any claim about how HACS packages, downloads, or registers this repo's release (comments, docs, `hacs.json`, upstream replies). Rule: cite `hacs/integration` file:line at a commit, or don't write it. |
