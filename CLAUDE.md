...
5. **`verify-install`** (second job in the same workflow) replays a HACS plugin install against the published release with `tools/hacs-install-sim.py` and fails the run if any runtime chunk the entry bundle requests is missing. Retries only for asset-propagation lag; a genuine missing chunk fails immediately. See [docs/RELEASE-INFRASTRUCTURE.md](docs/RELEASE-INFRASTRUCTURE.md#post-release-install-verification-gate).
